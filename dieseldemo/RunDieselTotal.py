import ProgramDieselTotal as PCT
import numpy as np
import pandas as pd
import datetime
import FunctionDiesel as FC
...
for i in range(1, epoch):
   ...
   rec_rmsec, rec_r_2_t, rec_rmsep, rec_r_2_p = PCT.ModelDieselTotal()
   write_data = [(i, now_s, epochs_au, batch_size_au, rec_rmsec, rec_r_2_t, rec_rmsep, rec_r_2_p)]  # 需要新写入的数据
   FC.write_To_Csv_Total(write_data)
   rmsec += rec_rmsec
   r_2_t += rec_r_2_t
   rmsep += rec_rmsep
   r_2_p += rec_r_2_p
...
df = pd.DataFrame(
   [('diesel_total', now_s, epochs_au, epoch - 1, avg_rmsec, avg_r_2_t, avg_rmsep, avg_r_2_p)])  # 列表数据转为数据框
df.to_csv('../resultsdemo/allResults.csv', mode='a', index=False, header=False)
df1 = pd.DataFrame([('best_diesel_total', max_r_2_p_label, best_now_s, epochs_au, batch_size_au,epoch - 1,
