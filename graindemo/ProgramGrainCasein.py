...
def ModelGrainCasein():
    ...
    callback_list_test =[
        keras.callbacks.ModelCheckpoint(
            filepath="./dataTimeGrainCasein/"+now_s+'.h5',      ##文件路径 存在当前路径下吧 还好找
            monitor='val_loss',         ## 监控指标
            save_best_only=True        ## 保持最佳模型
        )
    ]
    test_data,test_lable,train_data,train_lable = FC.getOdateGrainCasein()
    ...
    b_5 = layers.Conv1D(8, 3, activation=jihuo, padding='valid')(b_5)
    ...
    model = load_model("./dataTimeGrainCasein/"+now_s+'.h5')
